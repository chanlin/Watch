...
public class PageFactory {
    ...
    private int QHD = 960 * 540;
    private int HD720 = 1280 * 720;
    private int WXGA2 = 1790 * 1080;
    ...
    private int marginWeight = 50;
    // 字号
    private int mFontSize = 40;
    private int mNumFontSize = 35;
    private Bitmap m_book_bg;
    private Vector<String> m_lines = new Vector<String>();
    private Context mContext;
    private String mStringList;

    public PageFactory(Context context, String stringList,int w, int h) {
        mStringList=stringList;
        mWidth = w;
        mHeight = h;
		mFontSize = getFontSize();
        L.e("reader__mFontSize==="+mFontSize);
        mContext = context;
...
//		mNumPaint.setTypeface(typeface);
    ...
    public void onDraw(Canvas canvas,boolean isStart) {
        if (m_lines.size() == 0) {
            // first init
...
//			}else{
//				canvas.drawColor(mContext.getResources().getColor(R.color.reader_bg));
//			}
            ...
            if (isStart){
                canvas.drawText("0.00%", marginWeight + 2, mHeight - marginHeight, mNumPaint);
            }else{
...
//			canvas.drawText(mTime , mWidth - marginWeight - strLen, mHeight - marginHeight, mNumPaint);
    ...
    private void pageUp() {
        ...
        while ((lines.size() < mPageLineCount) && (m_mbBufBeginPos > 0)) {
            ...
            if (mStringList!=null)
                strParagraph = strParagraph.replace(mStringList, "");
    ...
    private Vector<String> pageDown() {
        ...
        while ((lines.size() < mPageLineCount) && (m_mbBufEndPos < m_mpBufferLen)) {
            ...
            if (mStringList!=null){
                ...
                strParagraph = strParagraph.replace(mStringList, "");
                strParagraph = strParagraph.replace("<#JSON#>","");
            }
            strParagraph = strParagraph.replaceAll("\r\n", "  ");
            strParagraph = strParagraph.replaceAll("\n", " ");
